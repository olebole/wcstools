 ...
 *** April 29, 2002
 *** By Doug Mink, dmink@cfa.harvard.edu (After IRAF mwcs/wfzpx.x)
...
/* ZPXINIT -- Initialize the zenithal/azimuthal polynomial forward or inverse
...
/* ZPXPOS -- Forward transform (physical to world) for the
	     zenithal/azimuthal polynomial projection. */

int
zpxpos (fc, p, w)

double  xpix, ypix;	/* Input physical coordinates (x, y) */
struct WorldCoor *wcs;	/* Input pointer to WCS descriptor */
double  *xpos, *ypos;	/* Returned world coordinates (ra, dec) */
{
...
/* ZPXPIX -- Inverse transform (world to physical) for the
	     zenithal/azimuthal polynomial projection. */

int
zpxpix (xpos, ypos, wcs, xpix, ypix)

double  xpos, ypos;     /* Input world coordinates (ra, dec) */
struct WorldCoor *wcs;  /* Input pointer to WCS descriptor */
double  *xpix, *ypix;   /* Returned physical coordinates (x, y) */
{
...
/* Apr 29 2002	Translated SPP to C from IRAF wfzpx.x
 */
