 ...
 * ucacsize (filename)	    Return size of UCAC catalog file in bytes
 */
...
char ucaccd[64]="/data/astrocat/ucac1";
...
int	nlog;		/* 1 for diagnostics */
{
   ...
   if (sortmag == 2) {
    ...
    nz = ucaczones (rdec1,rdec2,nrmax,zlist,nlist,verbose);
    if (nz <= 0) {
...
/* Loop through star list */
	...
	if (gnum[jstar]-(double)zone > 0.0000001) {
	    if (zone != zone0) {
		if (starcat != NULL)
		    ucacclose (starcat);
	    ...
	    if (ucacstar (starcat, star, zone, jstar)) {
...
static int
ucaczones (dec1, dec2, nzmax, zones, nstar, verbose)
...

{
    ...
    char *line;
    char *str;
    int nchar=64;	/* Number of characters per line in table */
    int iz, iz1, iz2;
    int irow,i;
    int ntok;
    double spd1, spd2;
...
/* Set pathname for index table file */
    strcat (tabpath,"/zone.txt");

/* Read the index table */
    if ((buffer0 = getfilebuff (tabpath)) == NULL) {
	fprintf (stderr,"UCACZONE:  error reading region table %s\n",tabpath);
	return (0);
	}
...
/* Find first and last declination zone to search */
    ...
    /* Set buffer pointer to start of table */
    buffer = strsrch (buffer0, "----");

    i = 0;
    for (iz = iz1; iz <= iz2; iz++) {

	/* Read next line of region table */
	line = buffer + (iz * nchar);
	ntok = setoken (&tokens, line, "");
	if (ntok > 4) {
	    zones[i] = iz;
	    if (getoken (&tokens, 5, token, 15))
		nstar[i++] = atoi (token);
	    else
		nstar[i++] = 0;
	    }
...

{
    ...
    sprintf (temp, "/u1/z%03d", zone);
    strcat (ucacpath, temp);
    ...
    sc->nbent = 67;
    sc->nstars = ucacsize (ucacpath) / sc->nbent;
