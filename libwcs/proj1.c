...
char *
prjset_errmsg (errno)
int errno;		/* Error code returned by prjset() */
{
    if (errno == 0)
	return ( (char) 0);
    else if (errno == 1)
...
char *
prjfwd_errmsg (errno)
int errno;		/* Error code returned by prjfwd() */
{
    if (errno == 0)
	return ( (char) 0);
    else if (errno == 1)
	...
	return ( "Invalid value of (phi,theta)" );
    else
	return ( "Unknown error code" );
}
...
char *
prjrev_errmsg (errno)
int errno;		/* Error code returned by prjrev() */
{
    if (errno == 0)
	return ( (char) 0);
    else if (errno == 1)
