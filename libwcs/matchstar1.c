...
static int	cdfit = 0;	/* 1 if CD matrix is to be fit */
...
static int	npvfit;	/* Number of projection parameters to fit */
static int	pvfit[10]; /* Projection parameters being fit */
...

{
    ...
    /* Get projection parameters to fit from digits of vpfit */
    sprintf (vpar, "%d", pfit);
    nfit0 = nfit;
    pvfit[0] = -1;
    for (i = 9; i < 19; i++) {
	...
	if (npvfit > 0) {
	    for (i = 0; i < 10; i++) {
		if (pvfit[i] > -1)
		    fprintf (stderr," pv%d= %8g ",wcs->prj.p[i]);
		}
	    }
...
int	nbin;	/* Number of points to be fit */
{
    int pfit;
